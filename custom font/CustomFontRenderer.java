...
public class CustomFontRenderer extends CustomFont{
	...
	public float drawStringWithSlightShadow(String text, double x, double y, Color color) {
		float shadowWidth = drawString(text, x + 0.3D, y + 1D, color, true);
		return Math.max(shadowWidth, drawString(text, x, y, color, false));
	}
	...
	public float drawCenteredStringWithSlightShadow(String text, float x, float y, Color color) {
		return drawStringWithSlightShadow(text, x - getStringWidth(text) / 2, y, color);
	}
	...
	public float drawString(String text, double x, double y, Color gsColor, boolean shadow) {
		...
		if (color.getAlpha()<4) color= new Color(color.getRed(), color.getBlue(), color.getGreen(), 255);
